...
export const api = axios.create({
  baseURL: process.env.REACT_APP_API_URL || "https://eventnow.mockapi.io/api",
  headers: {
...
export const fetchEventById = async (id) => {
  try {
    ...
    // Return mock data for demo purposes
    return mockEvents.find((event) => event.id.toString() === id.toString()) || null
  }
}

export const fetchRelatedEvents = async (category, currentEventId) => {
  try {
    const response = await api.get(`/events/related?category=${category}&eventId=${currentEventId}`)
    return response.data
  } catch (error) {
    console.error("Error fetching related events:", error)
    // Return mock data for demo purposes
    return mockEvents
      .filter((event) => event.category === category && event.id.toString() !== currentEventId.toString())
      .slice(0, 4)
  }
}
